...
; http://rainwarrior.ca
; Modifications by cppchriscpp 1/31/2022 to make it templated for
; Use in create-nes-game. 
; TODO: Do I keep everything here, or make it an extremely simple hello world? 
; Or maybe give the option between a simple hello world and some sample code?
;
...
INES_MAPPER = <%= mapperNumber %> ; <%= mapperNumber %> = <%= mapperName %>
INES_MIRROR = <%= mirrorMode === 'horizontal' ? 0 : 1 %> ; 0 = horizontal mirroring, 1 = vertical mirroring
INES_SRAM   = <%= useSram %> ; 1 = battery backed SRAM at $6000-7FFF

.byte 'N', 'E', 'S', $1A ; ID
.byte <%= prgBankCount %> ; 16k PRG chunk count
.byte <%= chrBankCount %> ; 8k CHR chunk count
